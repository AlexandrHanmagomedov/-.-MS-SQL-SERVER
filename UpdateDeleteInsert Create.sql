...
--FROM Polyclinic_HAZ.dbo.Students+
		...
		--update ,delete,insert -запросы для модификации данных
		--create(создания),alter(изменения) --операторы изменения структуры данных 
...
--VALUES (N'Мишин',N'Леонид','05-01-2010',null,N'leonid@mail.ru')
	 ...
	 --   Email nvarchar(100) NOT NULL unique)
		...
		--Копирование (или выборка) в новую базу
		--insert into StudentsCopy(LastName,FirstName,BirthDate,Grants,Email)
		--select LastName,FirstName,BirthDate,Grants,Email FROM Students

		--Изменить фамилию студента с Бублик на Баранку



		--UPDATE Students
		--SET LastName=N'Баранка'  --СЕТ УСТАНОВИТЬ НОВОЕ ЗНАЧЕНИЕ
		--WHERE ID=10

		--SELECT * FROM Students-- ВЫВОД НОВОЙ ИНФОРМАЦИИ
		--WHERE ID=10

		--ПРИБАВКА СТИПЕНДИИ ВСЕМ СТУДЕНТАМ
		--UPDATE Students
		--SET Grants=ISNULL(Grants,0)+10 --причем добавка и для null
		--FROM Students

		--SELECT*FROM Students

		--update ,delete,insert -запросы для модификации данных
		--create,alter --операторы изменения структуры данных 
